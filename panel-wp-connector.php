 ...
 * Version: 1.3.0
 * Author: Eric Nunes
 * Author URI: https://github.com/eric-nunes
...
spl_autoload_register(function($class) {
    ...
    foreach ($possible_files as $file) {
        if (file_exists($file)) {
            require_once $file;
            error_log("PanelWPConnector Autoloader: Loaded class {$class} from {$file}");
            return;
        }
    }

    // Log se a classe não for encontrada
    error_log("PanelWPConnector Autoloader: Could not find class {$class}");
});
